 ...
 * Uses identical visual components to AdjustFiltersScreen.
...
// ─── city data (identical to AdjustFiltersScreen) ─────────────────────────────
...
// ─── options (identical to AdjustFiltersScreen) ───────────────────────────────
...
// ─── shared sub-components (mirrors AdjustFiltersScreen exactly) ──────────────
...
function AgeStepper({
  label, value, min, max, onDecrement, onIncrement,
}: {
...
// ─── City Picker Modal (identical to AdjustFiltersScreen) ─────────────────────
...
export function PartnerPreferencesScreen({
  onBack,
  onSave,
  initialAgeMin = DEFAULTS.ageMin,
  initialAgeMax = DEFAULTS.ageMax,
  initialSect = 'Any',
  initialMaritalStatus = 'Any',
  ...
  const [heightMaxCm,     setHeightMaxCm]     = useState(185);
  ...
  const [sects,           setSects]           = useState<string[]>(
    initialSect && initialSect !== 'Any' ? [initialSect] : ['Any'],
  );
  const [minReligiosity,  setMinReligiosity]  = useState('Any');
  const [educationLevels, setEducationLevels] = useState<string[]>(['Any']);
  const [maritalStatuses, setMaritalStatuses] = useState<string[]>(
    initialMaritalStatus && initialMaritalStatus !== 'Any' ? [initialMaritalStatus] : ['Any'],
  );
  ...
  function resetAll() {
    ...
    setIncludeOverseas(DEFAULTS.includeOverseas);
    setSects(['Any']);
    setMinReligiosity('Any');
    setEducationLevels(['Any']);
    setMaritalStatuses(['Any']);
  }
  ...
  return (
    <View style={styles.root}>
      ...
      <CityPickerModal
        ...
        selected={cities}
        includeOverseas={includeOverseas}
...
// ─── styles (exact copy of AdjustFiltersScreen styles) ───────────────────────
