...
export default function App() {
  ...
  const [photoVisibility, setPhotoVisibility] = useState<string>('APPROVAL_REQUIRED');
  const [pausePhotoRequests, setPausePhotoRequests] = useState(false);
    ...
    Promise.all([getAccessToken(), getPendingEmail(), getPendingPhone()]).then(([token, pending, pendingPhone]) => {
      ...
      getMe()
        .then(async me => {
          ...
          if (me.profile.photoVisibility) setPhotoVisibility(me.profile.photoVisibility);
          if (me.profile.pausePhotoRequests !== undefined) setPausePhotoRequests(me.profile.pausePhotoRequests);
          ...
          const isWaliSession = me.user.role?.toLowerCase() === 'wali' ||
            WALI_STEPS.includes(me.profile.onboardingStep ?? '');
          if (isWaliSession) {
            ...
            const step = me.profile.onboardingStep as Screen | null;
      ...
      case 'WaliRole':
        return (
          <WaliRoleExplainScreen
            ...
            onAccept={async () => { await saveOnboardingStep('WaliDetails'); navigate('WaliDetails'); }}
          ...
          <WaliDetailsScreen
            ...
            onContinue={async (name, _relationship) => {
              ...
              await Promise.all([
                ...
                saveOnboardingStep('WaliComplete'),
              ]);
      ...
      case 'Privacy':
        return (
          <PrivacyScreen
            ...
            initialPhotoVisibility={photoVisibility}
            initialPauseRequests={pausePhotoRequests}
            onPhotoVisibilityChange={v => {
              setPhotoVisibility(v);
              updatePhotoPrivacy(v as PhotoVisibility).catch(() => {});
            }}
            onPauseRequestsChange={paused => {
              setPausePhotoRequests(paused);
              updatePhotoPrivacy(undefined, paused).catch(() => {});
            }}
          />
        );
